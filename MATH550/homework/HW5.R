...
library(car)
par(mfrow = c(1,1))
invResults <- invResPlot(model_golf)
# The Inverse response plot picked out a visually appropriate lambda-hat of about 0.04
# to use as an exponent int he power transform for PrizeMoney
...
golf$PrizeMoney <- golf$PrizeMoney^lambdaHat
golf
attach(golf)
pairs(PrizeMoney~., data = golf)
# The new pairs plot looks much better. Now, a rough linear relationship can be seen
...
model_golf <- lm(PrizeMoney ~., data=golf)
summary(model_golf)
# The adjusted R-squared value for the new power-transformed model is a lot better at 0.5441.
# Now, let's try a Log Transformation on Prize Money

# Log transforming PrizeMoney
golf$PrizeMoney <- log(golf$PrizeMoney)
golf
attach(golf)
pairs(PrizeMoney~., data = golf)
# The new pairs plot looks much better. Now, a rough linear relationship can be seen
...
# because of this, Iwill perform backwards step AIC and BIC to reduce the
# number of predictors and potential multicollinearity
n <- length(model_golf$residuals)
backAIC  <- step(model_golf,direction="backward", data=golf)
backBIC  <- step(model_golf,direction="backward", data=golf, k=log(n))
# The step model results driven by AIC give us back TKN, TS, and COD as the best predictors
# For BIC, I am given TS and COD
# Since there are already so few terms, the BIC approach might not be completely necessary
# It might be better to evaluate the models for each before making a decision on the best
...
# AIC
model_golf <- lm(PrizeMoney~SandSaves+Scrambling+GIR+BirdieConversion, data=golf)
summary(model_golf)
# BIC
model_golf <- lm(PrizeMoney~Scrambling+GIR+BirdieConversion, data=golf)
summary(model_golf)
pairs(PrizeMoney~SandSaves+Scrambling+GIR+BirdieConversion, data = golf)
# While the AIC has an additional term, the R-squared is slightly higher than using BIC
# When I go back to look at the problem statement, it does not specifically mention
# any previously understood relationship between TKN, TS, and COD.
# This suggests that perhaps the span of these three predictors will still be appropriate 
...
par(mfrow = c(2,2))
mmp(model_golf)
mmp(model_golf,SandSaves)
mmp(model_golf,Scrambling)
mmp(model_golf,GIR)
mmp(model_golf,BirdieConversion)
# the Marginal Model plots for the model and the individual predictors
# look good. The data tracks well with the proposed model


StanRes <- rstandard(model_golf)
par(mfrow = c(2,2))
plot(SandSaves,StanRes, ylab="Standardized Residuals",pch=19,col=10)
abline(h=0,lwd=2)
